...
import { PROMISE_STATUS, PROMISE_STRUCT, PROMISE_THEN, MyPromise } from './_promise';
const { Link } = Anchor;
  ...
  return (
    <article id="rootActicle" className={classMap.article}>
      <main className={classMap.content}>
        ...
        <a className={classMap.href} target="_blank" rel="noreferrer" href="https://promisesaplus.com/">
          Promises/A+规范
        </a>
        来一步步实现它。
        <br />
        ...
        promise必须是以下三种状态之一：<code>pending</code>,<code>fulfilled</code>,<code>rejected</code>
        <ul>
          <li>1. pending: promise可以变成fulfilled或者rejected</li>
          <li>2. fulfilled: promise不能再变成其他状态，必须有一个不可变的值</li>
          <li>3. rejected: promise不能再变成其他状态，必须有一个不可变的理由</li>
        </ul>
        ...
        <ul className={classMap.ul}>
          <li>
            如果onFulfilled是函数：在promise变为fulfilled状态之后，onFulfilled才能执行，<code>value</code>
            作为第一个参数，并且只能调用一次。
          </li>
          <li>
            如果onRejected是函数：在promise变为rejected状态之后，onRejected才能执行，<code>reason</code>
            作为第一个参数，并且只能调用一次。
          </li>
        </ul>
        ...
        <ul>
          <li>
            1. 如果onFulfilled或者onRejected返回了一个值<code>x</code>，执行Promise消除程序
            <code>[[Resolve]](promise2, x)</code>
          </li>
          <li>2. 如果onFulfilled或者onRejected抛出了异常，promise2必须被rejected，reason就是e</li>
          <li>
            3. 如果onFulfilled不是函数且promise1已经是fulfilled状态了,promise2也必须变为fulfilled，且value和promise1一样
          </li>
          <li>4. 如果onRejected不是函数且promise1已经rejected了，promise2也必须变为rejected，且reason和promise1一样</li>
        </ul>
        ...
        Promise解决程序接收promise和value作为参数，标记为<code>[[resolve]](promise,x)</code>
        ...
        <ul>
          ...
          <li>
            ...
            <ul className="pl-60">
              <li>2.1. 如果x是pending状态，promise必须保持pending直到变为fulfilled或者rejected</li>
          ...
          <li>
            ...
            <ul className="pl-60">
              ...
              <li>
                ...
                <ul className="pl-60">
                  <li>
                    3.3.1. 如果resolvePromise被调用了，值为y，执行<code>[[resolve]](promise,y)</code>
                  </li>
                  <li>
                    3.3.2 如果rejectPromise被调用了，reason为r，执行<code>reject(r)</code>
                  </li>
                  <li>3.3.3 如果两个都被调用了，或者多次用相同的参数调用。第一次调用优先，后续的忽略。</li>
                  <li>
                    3.3.4 如果调用then时抛出异常e，判断resolvePromise或者rejectPromise是否已执行，如果执行了就忽略，否则
                    <code>reject(e)</code>
                  </li>
                </ul>
              </li>
        ...
        <Anchor className="anchor" getContainer={() => document.getElementById('content') as HTMLElement}>
          <Link href="#promise" title="Promise"></Link>
          <Link href="#status" title="Promise状态">
            <Link href="#standard1" title="规范"></Link>
            <Link href="#code1" title="实现"></Link>
          </Link>
          <Link href="#then" title="then">
            <Link href="#standard2" title="规范"></Link>
            <Link href="#code2" title="实现"></Link>
          </Link>
          <Link href="#promiseResolutionProcedure" title="Promise解决程序">
            <Link href="#standard3" title="规范"></Link>
            <Link href="#code3" title="实现"></Link>
          </Link>
