  ...
  return (
    <article id="rootArticle" className={classMap.article}>
      <main className={classMap.content}>
        ...
        <br />
        <br />
        可以通过<code>window.performance.timing</code>
        获取加载过程模型中的各阶段耗时数据
        {timing}
        后来该属性被标记为废弃，改用
        <code>window.performance.getEntriesByType(&quot;navigation&quot;)</code>
        ...
        <ul className={classMap.ul}>
          <li>
            <code>FP(first paint)</code>
            ，表示页面开始首次绘制的时间点，值越小越好。在<code>FP</code>
            之前，用户看到的是导航之前的页面。
          </li>
          <li>
            <code>
            ...
            六大指标之一，表示首次绘制任何文本、图像、非空白<code>canvas</code>
            或<code>SVG</code>
            的时间点，值越小越好。
          </li>
        </ul>
        这两个指标，可以通过<code>performance.getEntry</code>、
        <code>performance.getEntriesByName</code>、
        ...
        <h3 id="tti" className={classMap.articleSubTitle}>
          何时可以交互 - TTI & TBT
        </h3>
        <ul className={classMap.ul}>
          <li>
            <code>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                href="https://web.dev/i18n/zh/tti/"
              >
            ...
            <br />
            官方没有提供接口获取，而是给出了计算公式，并且不建议手动测量该指标，而是建议获取
            <code>FID</code>
            ，具体点击上面的链接查看。
          </li>
          <li>
            ...
            ，总的阻塞时间，<code>lighthouse</code>六大指标之一，用于测量
            <code>FCP</code>到<code>TTI</code>
            之间的总阻塞时间，值越小越好。
            <br />
            同样是需要手动计算的时间，在计算<code>TTI</code>遍历收集
            <code>longTask</code>的同时，计算阻塞时间的总和。
          </li>
        </ul>
        <h3 id="fid" className={classMap.articleSubTitle}>
          交互是否有延迟 - FID & Long Task
        </h3>
        衡量交互是否有延迟有2个指标：<code>FID</code>、<code>Long Task</code>
        。其中<code>FID</code>
        用来衡量用户首次交互延迟的情况，<code>Long Task</code>
        用于衡量用户在使用应用的过程中遇到的延迟、阻塞情况。
        <ul className={classMap.ul}>
          <li>
            <code>
              <a
          ...
          <li>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              href="https://developer.mozilla.org/zh-CN/docs/Web/API/PerformanceLongTaskTiming"
            >
              Long task
            </a>
            ，这个指标可以告诉我们哪些任务执行耗费了50ms以上的时间。
            <ul className={classMap.ul}>
              任何连续不间断且主UI线程繁忙50ms及以上的时间区间。例如下面的场景：
              <li>长耗时的事件回调</li>
              <li>代价高昂的回流和其他重绘</li>
              <li>浏览器在超过50ms的事件循环相邻循环之间所做的工作</li>
            </ul>
            {longTaskCode}
        ...
        如果使用<code>lighthouse</code>
        进行分析，会使用6个指标。以下是我的网站分数
        ...
        {sentryInit}
        其中<code>dsn</code>需要在sentry新增project之后才能获取到，
        <code>tracesSampleRate</code>
        决定了上报的频率，配置为0则不上报。
        <h3 id="how" className={classMap.articleSubTitle}>
          原理
        </h3>
        原理就是如前文所述，通过<code>window.performance.getEntries</code>和
