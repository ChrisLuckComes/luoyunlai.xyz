...
class ModuleHtmlPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap(id, (compilation) => {
      ...
      this.htmlWebpackPlugin
        ...
        .alterAssetTags.tapAsync(id, (data, cb) => {
  ...
  dynamicInsertScript(id, compilation) {
    ...
    this.htmlWebpackPlugin
      .getHooks(compilation)
      .afterTemplateExecution.tap(id, (data, cb) => {
        let targetTags =
          data.plugin.options.inject === "head" ? "headTags" : "bodyTags";
        ...
        var morderScriptStr = JSON.stringify(
          data[targetTags]
            ...
            .filter((item) => item && !item.src.includes("runtime"))
        );
        var legacyScriptStr = JSON.stringify(
          data[targetTags]
            .map((item) =>
              item.attributes && item.attributes.src && item.attributes.nomodule
                ? { src: item.attributes.src }
                : null
            )
            .filter((item) => item && !item.src.includes("runtime"))
        ...
        if (morderScriptStr !== "[]") {
          data.headTags[0].innerHTML &&
            (data.headTags[0].innerHTML = data.headTags[0].innerHTML.replace(
              "${modernList}",
              morderScriptStr
            ));
